...
Mongodb的索引也是以B-Tree的形式实现的（但是不知道是B还是B+）。至于是聚集索引还是非聚集索引，没有找到更多的资料。实验了一下，在一个mongo数据库中新建一个collection然后插入一条文档，在对应的数据库文件夹下多了一个collection文件，同时多了一个index文件，所以**我猜**mongo的实现是非聚集式的。
